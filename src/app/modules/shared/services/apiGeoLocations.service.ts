...
export class GeoLocationsService {
  ...
  createLocations(post: StreetMapData) {
    ...
    const requestBody: CreateLocationRequest = {
        ...
        mapUrl: post.results[0].annotations.OSM.url,
        flag: post.results[0].annotations.flag,
        country: post.results[0].components.country,
        state: post.results[0].components.state,
        city: post.results[0].components.city,
        ...
        countryCode: post.results[0].components.country_code,
        county: post.results[0].components.county,
        formatted: post.results[0].formatted
      };
  ...
  findLocationByLatAndLng(lat: number, lng: number): Observable<any> {
    const url = `${this.baseUrl}/locations?lat=${lat}&lng=${lng}`;
